...
import { ShoppingBag, Heart, Star, X, Plus, Minus, AlertCircle } from "lucide-react";
...
export function ShopClient({ products, featured }: Props) {
  ...
  const handleBuyNow = (product: MerchProduct) => {
    if (!product.inStock) {
      setAlertMsg("This item is currently out of stock.");
      return;
    }
    setAlertMsg("Purchase link unavailable — item does not have an external link yet.");
  };
  ...
  return (
    <div className="mx-auto max-w-screen-2xl px-4 lg:px-6 py-8">
      {/* Alert */}
      {alertMsg && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-foreground text-background px-4 py-3 rounded-xl shadow-lg max-w-sm w-full mx-4">
          <AlertCircle className="h-4 w-4 shrink-0" />
          <span className="text-sm flex-1">{alertMsg}</span>
      ...
      {cartOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-sm bg-background border-l shadow-xl flex flex-col">
            ...
            {cart.length > 0 && (
              <div className="p-4 border-t space-y-3">
                ...
                <Button variant="bid" size="lg" className="w-full" onClick={() => handleBuyNow(cart[0]!.product)}>Checkout</Button>
